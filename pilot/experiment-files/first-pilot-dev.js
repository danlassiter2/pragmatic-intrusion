...
NOTE: This is the developer version, which creates a random participant ID (but
has the code in for extracting Prolific IDs), only runs 12 test trials, and has
all the console.logs in. Otherwise the two files (first-pilot-dev.js and 
first-pilot.js) should be identical in terms of the actual code (but perhaps not
comments and notes etc.).
...
This version defines the stims in the experiment code. Future version may want to 
read from csv instead. If so, this section here should contain info about this, 
e.g. mention what the structure of the csv is and what the structure of the 
read-in data is (i.e. when imported), following OELS practice.
...
var jsPsych = initJsPsych({
    on_finish: function () {
        var all_data = jsPsych.data.get(); //get all data as a backup
        ...
        //window.location = "https://app.prolific.com/submissions/complete?cc=CCJBI1KV"; // url to send ppts back to Prolific UNCOMMENT before piloting!
    }
});
...
// NOTE: could ADD in the testing stims, as they will be shown to every ppt so 
// guaranteed they'll need to be preloaded. Not done here but could be a way 
// to tidy up code at some later point.
...
// Will change to this to extract Prolific IDs: UNCOMMENT before posting on Prolific
...
REMOVE the following from non-dev version
...
the target, in practice means that it is chosing from all images with that content type since there are only 4 images per 
...
for (target_content_type of target_content_types) {
    //++test_trial_counter; // start the loop by adding 1 to this variable, so that the first trial ends up with count 1
...
// might want to add a check of if "Yes" to bilingual, require final question. Currently does not require this.
